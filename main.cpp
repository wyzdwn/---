...
void Game()
{
    ...
    first: //第一次进入循环先向默认方向前进
        ...
        {
        case UP: //方向键：上
            run(0, -1, tmp); //向上移动（横坐标偏移为0，纵坐标偏移为-1）
            
            break;
        case DOWN: //方向键：下
            run(0, 1, tmp); //向下移动（横坐标偏移为0，纵坐标偏移为1）
            
            break;
        case LEFT: //方向键：左
            run(-1, 0, tmp); //向左移动（横坐标偏移为-1，纵坐标偏移为0）
            
            break;
        case RIGHT: //方向键：右
            run(1, 0, tmp); //向右移动（横坐标偏移为1，纵坐标偏移为0）
